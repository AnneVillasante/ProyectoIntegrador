...
const templates = require('../utils/reportTemplate'); // <--- CORRECCIÓN: El archivo es 'reportTemplate.js' (singular)
...
const renderReport = (jsreport) => async (tipo, data) => {
  ...
  if (!templates[tipo]) {
    throw new Error(`La plantilla para el reporte de tipo "${tipo}" no existe en utils/reportTemplate.js`);
  }
